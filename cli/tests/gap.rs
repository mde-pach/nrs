...
fn report_creates_gaps_file_with_header() {
    ...
    assert!(content.contains("| Type | Target | Description |"), "should have table header");
    assert!(content.contains("| missing-context | src/billing/ | no context file for billing domain |"));
}
...
fn report_appends_to_existing_file() {
    ...
    assert_eq!(content.matches("| Type | Target | Description |").count(), 1);
    // Both rows present
    assert!(content.contains("| missing-context | src/billing/ | first gap |"));
    assert!(content.contains("| wrong | src/orders/ | second gap |"));
}
...
fn report_preserves_duplicates() {
    ...
    let content = std::fs::read_to_string(tmp.path().join("nrs.gaps.md")).unwrap();
    assert_eq!(
        content.matches("| missing-pattern | src/billing/services/ | retry strategy not documented |").count(),
        3,
