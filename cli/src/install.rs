...
const SKILL_MD: &str = include_str!("../../skill/nrs-setup/SKILL.md");
...
fn install_claude() -> Result<()> {
    ...
    let skills_dir = PathBuf::from(&home).join(".claude/skills/nrs-setup");
    let refs_dir = skills_dir.join("references");

    std::fs::create_dir_all(&refs_dir)
        .with_context(|| format!("failed to create {}", refs_dir.display()))?;

    std::fs::write(skills_dir.join("SKILL.md"), SKILL_MD)?;
    std::fs::write(refs_dir.join("context-formats.md"), CONTEXT_FORMATS)?;

    println!("installed Claude Code skill at {}", skills_dir.display());
    Ok(())
}
